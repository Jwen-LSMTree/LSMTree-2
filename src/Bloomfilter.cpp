...
bool BloomFilter::hasKey(uint64_t k) const
{
    ...
    hashValues = (uint32_t *)memset(hashValues, 0, 4 * sizeof(uint32_t));
    MurmurHash3_x64_128(&k, sizeof(k), 1, hashValues);
    return byteArray[*hashValues % BLOOM_FILTER_SIZE] & byteArray[*(hashValues + 1) % BLOOM_FILTER_SIZE] & byteArray[*(hashValues + 2) % BLOOM_FILTER_SIZE] & byteArray[*(hashValues + 3) % BLOOM_FILTER_SIZE];
}
